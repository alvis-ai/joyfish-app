...
class StoryGeneratingPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StoryGeneratingPage> createState() => _StoryGeneratingPageState();
}
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: const Icon(Icons.auto_stories_rounded, size: 54, color: AppTheme.skyDeep),
            ),
            SizedBox(height: 24.h),
            Text('正在编织今晚的故事', style: Theme.of(context).textTheme.displaySmall),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _InfoLine(
                      label: '尝试次数',
                      value: '${request?.attemptCount ?? 0}/${request?.maxAttempts ?? 0}',
                    ),
                    _InfoLine(label: '主题', value: request?.themeTags.join('、') ?? '未设置'),
            ...
            if (status == 'failed')
              AppButton.secondary(
                text: '返回重新编辑',
                onPressed: () => context.router.replace(const StoryComposerRoute()),
              ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          SizedBox(width: 72.w, child: Text(label, style: const TextStyle(color: AppTheme.mutedInk))),
